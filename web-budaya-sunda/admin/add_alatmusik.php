...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $asal_usul2 = trim($_POST['asal_usul2'] ?? '');
    $cara_pembuatan = trim($_POST['cara_pembuatan'] ?? '');
    $cara_permainan = trim($_POST['cara_permainan'] ?? '');

    if ($nama === '') {
        $errors[] = 'Nama alat musik harus diisi!';
    } else {
        // Cek apakah username dan email sudah ada
        $stmt = $conn->prepare("SELECT id_alatmusik FROM alatmusik WHERE nama = ?");
        $stmt->bind_param("s", $nama);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $errors[] = "Nama alat musik sudah ada!";
        }
        $stmt->close();
    }
    ...
    // Simpan ke database
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO alatmusik (nama, deskripsi, asal_usul2, cara_pembuatan, cara_permainan, gambar, created_at) 
                                VALUES (?, ?, ?, ?, ?, ?, NOW())");
        if ($stmt) {
            $stmt->bind_param('ssssss', $nama, $deskripsi, $asal_usul2, $cara_pembuatan, $cara_permainan, $gambar);
            if ($stmt->execute()) {
...
<div class="card">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea name="deskripsi" class="form-control"><?= htmlspecialchars($_POST['deskripsi'] ?? '') ?></textarea>
        </div>

        <div class="form-group">
            <label>Asal Usul</label>
            <textarea name="asal_usul2" class="form-control"><?= htmlspecialchars($_POST['asal_usul2'] ?? '') ?></textarea>
        </div>

        <div class="form-group">
            <label>Cara Pembuatan</label>
            <textarea name="cara_pembuatan" class="form-control"><?= htmlspecialchars($_POST['cara_pembuatan'] ?? '') ?></textarea>
        </div>

        <div class="form-group">
            <label>Cara Permainan</label>
            <textarea name="cara_permainan" class="form-control"><?= htmlspecialchars($_POST['cara_permainan'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <img id="preview" class="preview" src="uploads/alatmusik/default.png" alt="preview">
        </div>
